...
- [x] **P2-3 目录扩展（出版社配置外置）**
  ...
  *实现现状（2026-09）*：配置外置已落地（CLI/后端生效）；「管理出版社」Web 面板未实现，普通用户目前仍需手改 `pubs.json`。
  **【终版裁定】面板不再计划**——受众过窄（手改 JSON 即可达成），项目进入维护模式。
...
- [x] **P2-4 脚本化能力**
  ...
  *实现现状（2026-09）*：CLI `--export` 已支持 CSV/Markdown；Web UI 已提供「导出 CSV」（含截断时全量导出）。
  **【终版裁定】`--watch` 不再计划**——无人值守常驻与"个人学习备份"定位不符，且拉长维护面。

### Batch 3：集成与更新

- [x] **P2-5 应用内更新检查 + 一键下载（双通道，国内镜像可选）** 【已定方案】
  启动后异步查最新版本（仅取版本号，无遥测/PII，走系统代理）：
  - **版本源**：`https://api.github.com/repos/tau625/ShudaoLe/releases/latest`；国内网络不稳时自动回退镜像前缀（镜像地址可在设置中修改）
  - **有新版 → 界面顶部横幅**，两个动作可选：
    - 「查看新版」：跳转 Release 页（零风险基线档）
    - 「一键下载」：直接拉取 `ShudaoLe-X.Y.Z-windows-x64.zip` 到本地下载目录——默认 GitHub 直链，可配置 ghproxy 类镜像前缀加速国内下载；下载完成后**提示用户手动解压替换**（不自动替换）
  - **不做静默自动更新**：无签名 exe 自己替换自己是杀软误报 + 法律敏感的双重雷区。默认手动「检查更新」按钮，设置里可彻底关闭网络检查。
  *理由*：用户散落各地，没有更新通道就没有安全修复触达；国内用户直连 GitHub 慢/不稳，镜像通道决定该功能的实际可用性。*效果*：版本碎片收敛，P0 级修复能推到用户。

- [x] **P2-6 Windows 安装包（Inno Setup）** 【已定方案】
  GitHub Actions 的 windows-latest runner 自带 Inno Setup 6，CI 在打 zip 之后追加一步 `ISCC.exe installer.iss`，产出 `ShudaoLe-X.Y.Z-setup.exe` 挂到 Release。安装包提供：开始菜单/桌面快捷方式、标准卸载器、默认下载目录注册。
  **绿色 zip 并存保留**——教师群体很多用户偏好免安装版，两个通道都发。
  *理由*：zip 解压双击在普通教师眼里不像「正经软件」，装完没有卸载器也容易被安全软件标为可疑；setup.exe 显著降低使用门槛。*效果*：分发形态对标商业软件，后续可顺手提交 winget 社区清单（`winget install` 即装即升级，白嫖微软正经渠道）。

> **代码签名：已决策暂不购买**（Windows OV 约 ¥500-900/年、EV 约 ¥2500+/年，macOS $99/年）。
> 现阶段维持 README 的杀软白名单指引；若未来误报反馈集中，再优先考虑 Windows OV。
...
- [x] **P3-1 前端轮询合并**
  现有 4 个独立定时器（700ms 状态 / 1000ms 令牌 / 3000ms 残留 / 1500ms 计数）合并为统一刷新协调器（250ms 单心跳 + 任务登记表），按页面可见性（不可见 ×4 降频）与状态机（空闲 1500ms / 运行中 700ms）调节频率；令牌三处请求合一为 `fetchTokenStatus()`。各任务函数体、端点、DOM id 均未改动，元素校验 + 无头截图验证通过。
  *理由*：降低常驻开销与代码重复。*效果*：前端行为可预测、好调试。

- [x] **P3-2 GUI 路由表化**
  `do_GET`/`do_POST` 的 if-else 已改为 `GET_ROUTES`/`POST_ROUTES` 类级字典分发，各分支体逐行抽成 `_route_*` 方法（行为零变化），三处重复的 JSON body 解析收敛为 `_read_json_body()`；`pick_folder` 已是"单分发 + 三平台私有实现"形态，无需再动。10 项路由实测（含 404 与历史别名）全部通过。
  *理由*：端点已 10+，继续 if-else 不可维护。*效果*：加端点变成加一个方法 + 一行表项。

- [x] **P3-3 文档与开发笔记同步**
  README 目录结构补齐 P2 模块与安装包三件套，下载段加入 setup.exe，开发笔记新增 Inno Setup 两坑与 tag 重发处置（并修正拆包后的规则文件位置）；CHANGELOG.md 已建立（v1.0.0 ~ v1.3.0 全量）。
...
| Batch 3 | P2-5 更新检查+一键下载、P2-6 Windows 安装包 | P1 完成（可与 Batch 2 部分并行） |
...
- **国内镜像依赖**：更新下载的镜像前缀若指向第三方公益加速（ghproxy 类），存在镜像失效/被污染风险——默认走官方直链，镜像仅作为用户显式配置的加速选项，并在下载提示中展示实际来源。
