...
- **应用内自动更新**：更新横幅新增「自动更新」按钮——自动下载新版本更新包（实时进度、SHA256 完整性校验，校验不符自动删除重下），Windows 下载完成后一键静默安装（程序自动退出交由安装器接管）；macOS/Linux 下载完成后打开所在目录解压替换。失败时显示原因并可重试，不影响当前版本继续使用
...
- Linux 桌面快捷方式模板（`packaging/shudaole.desktop`），三端体验对齐

### 文档

- README 新增「下载被浏览器拦截」处理指南（保留下载/绕过 SmartScreen/误报申诉/哈希核对）
