...
> **📌 项目状态：功能已冻结，进入维护模式。** 自 v1.5.0 起不再新增功能，仅修复
> 平台变更导致的不可用问题。已知的平台依赖（目录接口格式、PDF 下载地址、登录
> 鉴权方式）若发生变更，工具会给出明确提示而非静默失败——遇到此类提示请到
> [Issues](https://github.com/tau625/ShudaoLe/issues) 反馈。
...
「一键更新」即可自动下载 + 安装（带进度与 SHA256 完整性校验）：

- **Windows**：下载完成后自动启动安装器，**会弹一次 UAC 确认提权**（写入 Program Files 必需）；
  同意后程序退出、由安装器完成升级、完成后自动启动新版本。如不希望弹 UAC，请改用「手动更新」
  用浏览器下载后自行决定何时安装
...
> 令牌获取逻辑已内置于 `shudaole` 包，由 GUI 直接 import，不再依赖外部进程）。
> 打包成 exe 后则无此要求。
...
> 网页界面覆盖日常下载的全部需求；少数高级能力目前仅在命令行侧提供：
> `--export`（书单导出 Markdown/CSV）、`--dry-run`（只解析不下载）、
> `--show-facets`（查看候选分布）、`--include-resources`（含课件资源条目）。
...
| 参数 | 说明 | 常用取值示例 |
|------|------|--------------|
| `--phase` | 学段 | 小学 / 初中 / 高中 / 特殊教育 |
| `--grade` | 年级 | 一年级…（初一→七年级） |
| `--subject` | 科目 | 语文 / 数学 / 英语 / 科学… |
| `--publisher` | 版本 | 分组名 `人教版系`（= 人教版+统编版+人教鄂教版）、真实标签 `统编版`、简写 `人教` 均可 |
| `--volume` | 册次 | 上册 / 下册 |
| `--audience` | 用途 | 学生用书（默认排除教师用书）/ 教师用书 |
| `--system` | 学制 | 六三学制 / 五四学制（`六·三` `54学制` 等口语写法均可） |
| `--school` | 学校类型 | 常规学校 / 特殊学校 / 盲校 / 聋校 / 培智 |
...
- **无代码签名**：安装包与 exe 未做数字签名，Windows SmartScreen 可能弹出「已保护你的电脑」——点「更多信息 → 仍要运行」即可；sha256 校验值见 Release 附件 `SHA256SUMS.txt`。签名需要商业证书（与 Noncommercial 许可冲突），故不计划解决。
- **平台依赖**：目录列表、PDF 下载地址、登录鉴权均依赖平台现行接口。平台改版时工具会给出明确提示（而非静默失败），但修复需要等新版本——项目处于维护模式，请到 Issues 反馈。
- **目录更新为全量**：刷新目录需重新下载全量索引（约 40MB），无增量更新；目录数据本地缓存 7 天。
- **命令行默认包含教师用书**：网页界面默认只看学生用书；CLI 检索需显式加 `--audience 学生用书` 排除教师用书。
...
> 后续改进规划见 [ROADMAP](ROADMAP.md)（P0 正确性修复 → 工程化地基 → 并发下载/断点续传/目录扩展/更新检查）。
